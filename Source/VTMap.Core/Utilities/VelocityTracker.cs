...
namespace VTMap.Core.Utilities
{
    public class VelocityTracker
    {
        ...
        readonly long maxTicks = TimeSpan.FromMilliseconds(200).Ticks;  // Use only events from the last 200 ms
        ...
        public (double vx, double vy) CalcVelocity(long id, long now)
        {
            ...
            for (var i = 1; i < eventsArray.Length; i++)
            {
                ...
                if (now - lastTime < maxTicks)
                {
                    ...
                    distanceX += (nowX - lastX) * TimeSpan.TicksPerSecond; // / (nowTime - lastTime);
                    distanceY += (nowY - lastY) * TimeSpan.TicksPerSecond; // / (nowTime - lastTime);
                }
