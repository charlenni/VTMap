            ...
            switch (element.Type)
            {
                case GeometryType.Point:
                    foreach (var point in element.Points)
                    {
                        ...
                        {
                            var iconTextSymbol = styler.CreateIconTextSymbol(point.ToSKPoint(), element.Tags, context);
                            if (iconTextSymbol != null)
                                Symbols.Add(iconTextSymbol);
                        }
                        else if (styler.HasIcon)
                        {
                            var iconSymbol = styler.CreateIconSymbol(point.ToSKPoint(), element.Tags, context);
                            if (iconSymbol != null)
                                Symbols.Add(iconSymbol);
                        }
                        else if (styler.HasText)
                        {
                            var textSymbol = styler.CreateTextSymbol(point.ToSKPoint(), element.Tags, context);
                            if (textSymbol != null)
